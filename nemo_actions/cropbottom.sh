...
CROP_PERCENT=$(zenity --entry \
    --title="Crop Video from Bottom" \
    --text="Enter crop percentage of height from the bottom (e.g. 13):" \
    --entry-text="13" \
    --width=400)
...
for f in "$@"; do
    ...
    # Show progress/info
    echo "Cropping ${CROP_PIXELS}px from bottom of $(basename "$f") → $(basename "$OUTFILE")"

    # Run ffmpeg (crop from bottom)
    ffmpeg -i "$f" \
           -vf "crop=in_w:in_h-${CROP_PIXELS}:0:0" \
...
zenity --info --title="Crop Complete" --text="Finished cropping ${CROP_PERCENT}% from the bottom of selected video(s)." --width=350
